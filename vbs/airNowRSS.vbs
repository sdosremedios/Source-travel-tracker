...
'       urlParams.add "url", "https://airnowapi.org/aq/observation/latLong/current?"
        urlParams.add "url", "http://feeds.airnowapi.org/rss/realtime/98.xml"
...
'       urlParams.add "lon", "&longitude=-122.1947"
        ...
        dim xmlElements : set xmlElements = xmlDoc.DocumentElement.GetElementsByTagName("description")
        if xmlElements.length > 1 then
            dataText = xmlElements(1).text
        end if
...
'       Log "URL", URL
	...
	private function httpGet(URL)
		...
		If (Err.Number <> 0) or (web.Status <> "200") Then 
			httpGet = web.responseXML
            Log "Web Request Failed", Err.Description
		else
